...
trait BackgroundJobRepository {
   ...
   *   background job
   * @return
   *   background job with inserted id
   */
   ...
   *   at least when should be the last attempt
   * @return
   */
  ...
  /**
   * Change background job status to "Started" "respecting concurrency" which means that if 2
   * instances/processes/threads will call this method with the same id only one result should be true
   *
   * @param bgJobId
   *   background job id
   * @param newTryCount
   ...
   * @param now
   *   current date
   * @param betweenAttemptInterval
   *   how long to wait after a "failed attempt" before trying again
   * @return
   *   future with the result: true - job is started by this instance false - job is started by someone else
   *   (concurrently)
   */
  ...
  /**
   * Delete background jobs that were succeeded or failed before certain date
   *
   * @param date
   *   finish date upper bound
   * @return
   */
